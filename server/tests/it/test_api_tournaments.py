...
class ApiTournamentsIT(QuadradiusIntegrationTestCase):
    ...
    async def _make_two_users(self) -> Tuple[DbUser, DbUser]:
        ...
        if not user1:
            raise ValueError('Failed to create user 1')
        ...
        if not user2:
            raise ValueError('Failed to create user 2')

        return (user1, user2)

    async def test_list_tournament_users_tournament_exists(self):
    ...
    async def test_list_tournament_users_both_exist(self):
        ...
        async with api_client.get(f'tournaments/{tournament_id}/users') as r:
            ...
            actual_users = {frozenset(u.items()) for u in actual_users['users']}
            expected_users = {frozenset(u.items()) for u in expected_users['users']}

            self.assertEqual(actual_users, expected_users)

    async def test_list_tournament_users_tournament_does_not_exist(self):
        api_client = await self.new_api_client('v1')
    ...
    async def test_get_tournament(self):
        created_at = datetime(2020, 1, 2, 12, 0, 0, tzinfo=timezone.utc)
        started_at = datetime(2020, 1, 3, 12, 0, 0, tzinfo=timezone.utc)
        ...
        async with api_client.get(f'tournaments/{tournament_id}') as r:
            self.assertEqual(r.status, 200)
            self.assertEqual(await r.json(), {'tournament': {
    ...
    async def test_get_tournament_does_not_exist(self):
        api_client = await self.new_api_client('v1')
        async with api_client.get('tournaments/nonexistent') as r:
            self.assertEqual(r.status, 404)
            self.assertEqual(await r.json(), None)

    async def test_get_tournament_duels_tournament_exists(self):
        tournament_id = await self.connector.create_tournament('test_name', '123', '234', 3)
    ...
    async def test_get_tournament_duels_both_exist(self):
        tournament_id = await self.connector.create_tournament('test_name', '123', '234', 3)
        user1, user2 = await self._make_two_users()

        result = await self.connector.add_duel(
            tournament_id, duel_idx=0, active_until=datetime(2020, 12, 1, 1, 1, 1, tzinfo=timezone.utc),
    ...
    async def test_get_tournament_duels_tournament_does_not_exist(self):
        api_client = await self.new_api_client('v1')
        async with api_client.get('tournaments/nonexistent/duels') as r:
            self.assertEqual(r.status, 404)
            self.assertEqual(await r.json(), None)

    async def test_get_tournament_duel_matches_tournament_exists(self):
        tournament_id = await self.connector.create_tournament('test_name2', '123', '234', 3)
        api_client = await self.new_api_client('v1')

        async with api_client.get(f'tournaments/{tournament_id}/matches') as r:
            self.assertEqual(r.status, 200)
            self.assertEqual(await r.json(), {'matches': {}})

    async def test_get_tournament_duel_matches_both_exist(self):
        tournament_id = await self.connector.create_tournament('test_name', '123', '234', 3)
        user1, user2 = await self._make_two_users()
    ...
    async def test_get_tournament_duel_matches_tournament_does_not_exist(self):
        api_client = await self.new_api_client('v1')
