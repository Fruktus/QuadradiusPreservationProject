...
class ApiOauthIT(QuadradiusIntegrationTestCase):
    ...
    async def _login(self, client, username='testuser', password='asd'):
        async with client.post('/api/oauth/token', json={
            'grant_type': 'password',
            'username': username,
    ...
    async def test_wellknown(self):
        ...
        self.assertEqual(doc['token_endpoint'], 'http://localhost/api/oauth/token')
        self.assertEqual(doc['userinfo_endpoint'], 'http://localhost/api/oauth/userinfo')
