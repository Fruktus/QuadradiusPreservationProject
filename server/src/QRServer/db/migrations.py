...
async def _migration_upgrade_to_v12(c, _config):
    await c.execute(
        ...
        " challenger_tmp_pass varchar,"
        " challenged_tmp_pass varchar,"
...
async def _migration_invite_test(c, _config):
    ...
    try:
        ...
        print(f'DBG INVITE LINK: http://localhost:8000/challenge?id={invid} valid until {(datetime.now(timezone.utc) + timedelta(minutes=10))}')
    except Exception:
