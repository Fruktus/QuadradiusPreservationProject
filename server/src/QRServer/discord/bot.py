...
class DiscordBot:
    ...
    async def run_bot(self):
        ...
        async def create_tournament(interaction, tournament_name: str, dc_msg_id: str, required_matches_per_duel: int):
        ...
        @discord.app_commands.describe(
            ...
            active_until="Date until which matches for first round are accepted, in the isoformat."
                         " Ex: 2021-07-27T16:02:08.070557")
        ...
        @discord.app_commands.describe(username="The username to add as participant")
        async def join_tournament(interaction, username: str):
    ...
    async def _leave_tournament(self, interaction, username: str) -> None:
        """
            ...
            The command may fail if the tournamend_name is invalid, the username does not belong to the callee
            (or is banned/invalid), if it was not registered, or the tournament has already began.
        """
        user = await self.connector.get_user_by_username(username)
        # TODO include bans here as well
        if not user or user.discord_user_id != interaction.user.id:  # Do not disclose whether account exists or not
    ...
    async def _start_tournament(self, interaction, tournament_name: str, active_until: str) -> None:
        ...
        """
        try:
            active_until = self._parse_date(active_until)
        ...
        # Populate the duels
        await self._start_round_from_participants(tournament.tournament_id, indexer.levels, participant_pairs,
    ...
    async def _start_round_from_participants(self, tournament_id: str, level: int, new_participant_pairs: list,
        ...
        """
            ...
            Does not re-order the participants in any way, inserts them from lower tree index to higher
    ...
    def _parse_date(date_str: str) -> datetime:
        """
        Raises:
            ValueError if the provided string could not be parsed
        """
        return datetime.strptime(date_str, "%Y-%m-%d").replace(tzinfo=utils.tz_aoe())
